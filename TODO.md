...
| ⬜ To Do | Ability art | All 8 abilities (`ABILITY_CFG` in client.js: Sprint/Dash/Tongue/Slow Time/Sidekick/Armor/Magnet/Weighted Ring) currently have no dedicated art — level-up cards are text-only. Need an icon per ability (and ideally per level/tier) for the level-up cards, plus any in-world indicator icons (e.g. the Dash ready-ring is a plain canvas-drawn circle right now). |
